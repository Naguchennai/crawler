...
//    foreach($this->links_found_url_descriptors as $dd){
//        $v1[] = $dd->refering_url;
//    }
    
    // Array count same links
    //$v1 = (array_count_values($v1));

    for ($x=0; $x<$cnt; $x++)
    {
      // Set array link in new element array in count same links
      //$this->links_found_url_descriptors[$x]->refering_sum = $v1[$this->links_found_url_descriptors[$x]->refering_url];
      $UrlDescriptor = $this->links_found_url_descriptors[$x];

      $object_vars = get_object_vars($UrlDescriptor);

      $this->links_found[] = $object_vars;
      //$this->links_found[$x]['refering_sum'] = $v1[$this->links_found[$x]['refering_url']];
    }
  }
